...
export default function PopUpEditPlaylist({ setShowPopUpEdit, playlist }) {
    ...
    const [ midiasPlayList, setMidiasPlaylist ] = useState(playlist.midias)

    const filterMidias = midias.filter(midia => !midiasPlayList.find(midiaPlaylist => midiaPlaylist.midia_id === midia.midia_id))
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
              ...
              const midiasData = data.map(midia => ({
                midia_id: midia.id, 
                file_name: midia.file_name
            }));
                                        ...
                                        <div className="bg-white text-black h-72 border border-dark-blue pb-24 overflow-scroll">
                                            {midiasPlayList.map((midia, index) => (
                                                <div 
                                                    key={index} 
                                                    className="text-start my-2 py-1 px-2 hover:bg-dark-blue hover:text-white rounded-lg" 
                                                    onClick={() => setMidiasPlaylist(state => state.filter(midiaPlaylist => midiaPlaylist.midia_id !== midia.midia_id))}
                                                >
                                                    <span className="text-sm">{midia.file_name}</span>
                                                </div>
                                            ))}
